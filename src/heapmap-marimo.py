    ...
    class ActivationsAndGradients:
        ...
        def release(self):
            for handle in self.handles:
                handle.remove()
    return ActivationsAndGradients, letterbox
...
def _(
    ...
    class yolo_heatmap:
        ...
        def __call__(self, img_path, save_path="./results"):
            # remove dir if exist
            ...
            if os.path.isdir(img_path):
                ...
                return self.process(img_path, f'{save_path}/result.png')
    return (yolo_heatmap,)


@app.cell(hide_code=True)
def _(control_conf, control_layer1, control_layer2):
    def make_params(weight, m=1):
        return {
            'weight': weight,  # 训练出来的权重文件
            'device': 'cuda:0',
            'method': 'GradCAM',  # GradCAMPlusPlus, GradCAM, XGradCAM , 使用的热力图库文件不同的效果不一样可以多尝试
            'layer': [control_layer1.value if m==1 else control_layer2.value],  # 想要检测的对应层
...
def _(make_params, yolo_heatmap):
    model1 = yolo_heatmap(**make_params(
        "/icislab/volume3/benderick/futurama/YOLO-UAV/logs/YOLO-UAV/runs/2025-04-16_18-39-56-yolo11-EnhancedMAB-ODConv-ADown/YOLO-UAV/yolo11-EnhancedMAB-ODConv-ADown/weights/best.pt",
        1
    ))
    return (model1,)
...
def _(make_params, yolo_heatmap):
    model2 = yolo_heatmap(**make_params(
        '/icislab/volume3/benderick/futurama/YOLO-UAV/logs/YOLO-UAV/runs/2025-04-15_23-29-35-yolo11-SmallMAB-ODConv-ADown/YOLO-UAV/yolo11-SmallMAB-ODConv-ADown/weights/best.pt',
        2
    ))
    return (model2,)
